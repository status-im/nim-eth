...
iterator chunksForBloom(h: Hash256): array[2, uint8] =
...
iterator bloomBits(h: Hash256): UInt2048 =
...
proc incl*(f: var BloomFilter, h: Hash256) =
  for bits in bloomBits(h):
    f.value = f.value or bits

proc init*(_: type BloomFilter, h: Hash256): BloomFilter =
  result.incl(h)
...
proc contains*(f: BloomFilter, h: Hash256): bool =
  for bits in bloomBits(h):
