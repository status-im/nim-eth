...
proc installHandlers(
  ...
  srv.rpc("connect") do(r: enr.Record) -> SKey:
    let
      node = newNode(r).get()
      addRes = d.addNode(node)
      pingRes = await d.ping(node)
      connresult = await s.connectTo(node)

    if (connresult.isOk() and pingRes.isOk()):
...
proc buildAcceptConnection(t: ref Table[SKey, UtpSocket[Node]]): AcceptConnectionCallback[Node] = 
  return (
    proc (server: UtpRouter[Node], client: UtpSocket[Node]): Future[void] =
      ...
      t[key] = client
      fut.complete()
      echo "received connection from " & $client.socketKey
      return fut
  )
