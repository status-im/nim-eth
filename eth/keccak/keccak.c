...
void keccak_update(keccak_st *ctx, const uint8_t *in, size_t in_len) {
  ...
  if (ctx->absorb_offset != 0) {
    assert(ctx->absorb_offset < RATE_BYTES);
    size_t first_block_len = RATE_BYTES - ctx->absorb_offset;
...
void keccak_finish(keccak_st *ctx, uint8_t *out, size_t out_len) {
  ...
  size_t todo = 32;
  memcpy(out, state_bytes, todo);
}

void keccak256_20(const uint8_t *in, uint8_t *out) {
  // This function uses 500 units of time compared to
  // init+update+finish triplet 900 units of time
  keccak_st ctx;
  const int in_len = 20;
  const int skip_words = in_len / sizeof(ctx.state[0]);

  // initialize
  memset(&ctx.state[skip_words], 0, sizeof(ctx) - skip_words * sizeof(ctx.state[0]));

  // absorb
  memcpy(ctx.state, in, in_len);
...
void keccak256_32(const uint8_t *in, uint8_t *out) {
  // This function uses 200 units of time compared to
  // init+update+finish triplet 400 units of time
  // This function faster than keccak256_20 with shorter input length
  keccak_st ctx;
  const int in_len = 32;
  const int skip_words = in_len / sizeof(ctx.state[0]);

  // initialize
  memset(&ctx.state[skip_words], 0, sizeof(ctx) - in_len);
